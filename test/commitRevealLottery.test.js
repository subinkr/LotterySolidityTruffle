...
contract("CommitRevealLottery", accounts => {
  ...
  describe("constructor", () => {
    it("commitCloses & revealCloses should be set correctly", async () => {
      ...
      assert.equal(commitCloses.toString(), web3.utils.toBN(currentBlockNumber).add(duration).toString(),
  ...
  describe("Enter", () => {
    it("Should revert if a player enters less than 0.01 ether", async () => {
      ...
      const secret = 12345;
      const commit = web3.utils.keccak256(web3.utils.encodePacked(
        { value: accounts[1], type: "address" },
        { value: secret, type: "uint256" }
      ));
      console.log(`commit: ${commit}`);

      await truffleAssert.reverts(commitRevealLottery.enter(commit,
        { from: accounts[1], value: enterAmt }
        // 에러메세지가 enter의 require과 다르면 오류 발생, 안써도 됨
    ...
    it("Enter 3 players and check values", async () => {
      ...
      const secret1 = 12345;
      const commit1 = web3.utils.keccak256(web3.utils.encodePacked(
        { value: accounts[1], type: "address" },
      ...
      assert.equal(await commitRevealLottery.getBalance(), enterAmt), "0.01 ETH not sent correctly by account 1";
      assert.equal(await commitRevealLottery.commitments(accounts[1]), commit1);

      const secret2 = 12346;
      const commit2 = web3.utils.keccak256(web3.utils.encodePacked(
      ...
      const secret3 = 12347;
      const commit3 = web3.utils.keccak256(web3.utils.encodePacked(
      ...
      const secret4 = 12348;
      const commit4 = web3.utils.keccak256(web3.utils.encodePacked(
      ...
      await truffleAssert.reverts(
        commitRevealLottery.enter(commit4, { from: accounts[4], value: enterAmt }),
  ...
  describe("Reveal", () => {
    ...
    describe("WithdrawPrize", () => {
      it("WithdrawPrize", async() => {
        ...
        await truffleAssert.reverts(commitRevealLottery.withdrawPrize({ from: accounts[0] }), "You're not winner");
        ...
        assert.equal(commitCloses.toString(), web3.utils.toBN(currentBlockNumber).add(duration));
        assert.equal(revealCloses.toString(), commitCloses.add(duration));
      });
    });
  });
